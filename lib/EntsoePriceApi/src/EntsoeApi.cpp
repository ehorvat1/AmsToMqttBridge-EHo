...
PricesContainer* EntsoeApi::fetchPrices(time_t t) {
    ...
    if(strlen(getToken()) > 0) {
        ...
        if(http.begin(buf)) {
            ...
            if(status == HTTP_CODE_OK) {
                if(debugger->isActive(RemoteDebug::WARNING)) debugger->printf_P(PSTR("Receiving data\n"));
                data = http.getString();
                http.end();
                ...
                if(gcmRet > 0) {
                    ...
                    if(debugger->isActive(RemoteDebug::ERROR)) debugger->printf_P(PSTR("(EntsoeApi) +++++--- punkt 19= %d  punkt 20= %d\n"), (ret->points[19]),(ret->points[20]));  //EHorvat get prices
                    if(debugger->isActive(RemoteDebug::ERROR)) debugger->printf_P(PSTR("(EntsoeApi) +++++--- punkt 19= %f  punkt 20= %f\n"), (ret->points[19]),(ret->points[20]));  //EHorvat get prices
                    lastError = 0;
                    nextFetchDelayMinutes = 1;
                    return ret;
                } else {
