...
//	server.on(F("/consent"), HTTP_GET, std::bind(&AmsWebServer::indexHtml, this)); //EHorvat disabled consent stuff
	...
	server.on(F("/data.json"), HTTP_GET, std::bind(&AmsWebServer::dataJson, this));			//EHorvat data.json changed...added fixed income "pi" after line 33
	server.on(F("/minuteplot.json"), HTTP_GET, std::bind(&AmsWebServer::minuteplotJson, this));  //EHorvat new minute plot
...
//	bool access = WiFi.getMode() == WIFI_AP || webConfig.security < level;	    //EHorvat disabled this line ....see next line
...
//		sys.dataCollectionConsent,			//EHorvat disabled consent stuff  NOTE: also delete line 10 ("fwconsent": %d,) from sysinfo.json...chrashes if not
		...
		ui.showPeaks,
		ui.showPricePlot,
		ui.showMinutePlot,	//EHorvat new minute plot
...
void AmsWebServer::dataJson() {
	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Serving /data.json over http...\n"));//EHorvat changed to Info level
	uint64_t millis = millis64();

	if(!checkSecurity(2, true))
		return;
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of data.json...is: %d, \n"), strlen(buf));//EHorvat
	float vcc = hw->getVcc();
	...
	float refund = ea->getIncomeRefundForHour(0);         //EHorvat new fixed income price / refund
    String last_cntr_info = ea->getLast_Counter_Info();   //EHorvat new last_counte_value 
    String last_expcntr_info = ea->getLast_expCounter_Info();   //EHorvat new last_counte_value 
	...
	snprintf_P(buf, BufferSize, DATA_JSON,
		...
		refund == ENTSOE_NO_VALUE ? "null" : String(refund, 2).c_str(),		//EHorvat new fixed income price / refund Link in data.json:"pi" : %d,
		ea->getLast_Counter_Value(),              //EHorvat new last_counte_value ... Link in data.json:"py" : %lu,
		last_cntr_info.c_str(),                   //EHorvat new last_counte_value ... Link in data.json:"pd" : "%s",
		ea->getCostsLastCounterValue(),			  //EHorvat new last_counte_value ... Link in data.json:"pl" : %.2f,
		ea->getLast_Counter_Diff(),               //EHorvat new last_counte_value ... Link in data.json:"pa" : %lu,
		ea->getLast_expCounter_Value(),           //EHorvat new last_counte_value ... Link in data.json:"pz" : %lu,
		last_expcntr_info.c_str(),                //EHorvat new last_counte_value ... Link in data.json:"pg" : "%s",
		ea->getCostsLastexpCounterValue(),		  //EHorvat new last_counte_value ... Link in data.json:"pj" : %.2f,
		ea->getLast_expCounter_Diff(),            //EHorvat new last_counte_value ... Link in data.json:"pk" : %lu,
		meterState->getMeterType(),
	...
	server.setContentLength(strlen(buf));
	server.send(200, MIME_JSON, buf);
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of data.json...is: %d, \n"), strlen(buf));//EHorvat wants more info
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("+++ Webserver +++++ eapi=: %s, \n"), eapi == NULL ? "false" : "true");//EHorvat wants more info
}
...
void AmsWebServer::dayplotJson() {
	...
	if(!checkSecurity(2))
		return;
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of dayplot.json...is: %d, \n"), strlen(buf));//EHorvat wants more info
	if(ds == NULL) {
		notFound();
	} else {
...
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of dayplot.json...is: %d, \n"), strlen(buf)); //EHorvat ...buffer used: 22 bytes per element
}
...
void AmsWebServer::monthplotJson() {
	...
	if(!checkSecurity(2))
		return;
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of monthplot.json...is: %d, \n"), strlen(buf)); //EHorvat wants more info
	if(ds == NULL) {
		notFound();
	} else {
...
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("++++++++getHour-179 : %i\n"), ds->getMinuteWatt(179));  //EHorvat changed to Info level
	...
	if(!checkSecurity(2))
		return;
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of minuteplot.json...is: %d, \n"), strlen(buf));//EHorvat
	if(ds == NULL) {
		notFound();
	} else {
...
void AmsWebServer::hourplotJson() {
	...
	if(!checkSecurity(2))
		return;
//	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Buffer length actual of hour.json...is: %d, \n"), strlen(buf));//EHorvat
	if(ds == NULL) {
		notFound();
	} else {
...
void AmsWebServer::energyPriceJson() {
	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Serving /energyprice.json over http...\n"));
...
void AmsWebServer::configurationJson() {
	...
	snprintf_P(buf, BufferSize, CONF_PRICE_JSON,
		...
		entsoe.fixedRefund == 0 ? "null" : String(entsoe.fixedRefund / 1000.0, 10).c_str(), //EHorvat new fixed income price / refund
		entsoe.last_counter_value,      //EHorvat new last_counter_value
		strncpy(entsoe.last_counter_info, entsoe.last_counter_info, 16),      //EHorvat new last_counter_value
		entsoe.last_expcounter_value,      //EHorvat new last_counter_value
		strncpy(entsoe.last_expcounter_info, entsoe.last_expcounter_info, 16)      //EHorvat new last_counter_value
...
//		sys.dataCollectionConsent = server.hasArg(F("sf")) && (server.arg(F("sf")) == F("true") || server.arg(F("sf")) == F("1")) ? 1 : 2;	//EHorvat disabled consent stuff
	...
	if(server.hasArg(F("u")) && server.arg(F("u")) == F("true")) {
		...
		ui.showMinutePlot = server.arg(F("uq")).toInt();  //EHorvat new minute plot
	...
	if(server.hasArg(F("p")) && server.arg(F("p")) == F("true")) {
		...
		entsoe.fixedRefund = server.arg(F("pi")).toFloat() * 1000;		         //EHorvat new fixed income price / refund
		entsoe.last_counter_value = server.arg(F("py")).toInt();		         //EHorvat new last_counter_value
		strncpy(entsoe.last_counter_info, server.arg(F("pd")).c_str(), 16);      //EHorvat new last_counter_value limited to 16 char
		entsoe.last_expcounter_value = server.arg(F("pz")).toInt();		         //EHorvat new last_counter_value
		strncpy(entsoe.last_expcounter_info, server.arg(F("pg")).c_str(), 16);   //EHorvat new last_counter_value limited to 16 char
	...
	if (config->save()) {
		...
		if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Error at EEPROM save....\n"));  //EHorvat new fixed income price / refund
		success = false;
	}
...
/*//EHorvat disabled consent stuff start
...
*/ //EHorvat disabled consent stuff end
...
void AmsWebServer::firmwarePost() {
	...
	if(rebootForUpgrade) {
		server.send(200);
	} else {	/*  //EHorvat disabled consent stuff start
...
//		server.sendContent(buf, snprintf_P(buf, BufferSize, PSTR("entsoeFixedRefund %.3f\n"), entsoe.fixedRefund / 1000.0));	//EHorvat new fixed income price / refund
	}
