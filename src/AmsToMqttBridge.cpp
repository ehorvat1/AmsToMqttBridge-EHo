...
unsigned long Last_APClient_millis =millis(); //EHorvat Used to auto AP swapp
...
//EHorvat new hour plot new start
	if(now > 10000 && now - Last_5second_millis > 5000) { 
		Last_5second_millis = now;
		if (meterState.getLastError() == 0) {
			import_export = meterState.getActiveImportPower() - meterState.getActiveExportPower();
		}
		summ_60minplot = summ_60minplot + import_export;
		index_60minplot++;
		ds.updateMinute(import_export);
		if(Debug.isActive(RemoteDebug::VERBOSE)) debugI("+++++ now millis: %i  +++ Minute Plot updateMinute was done with: %d Watt\r\n", now, import_export);

	}
	yield();
	if(now > 10000 && now - Last_hourplot_avg > 30000) {
	    ...
	    if (index_60minplot > 0) {
		    ...
		    index_60minplot = 0;
			summ_60minplot = 0;
		    if(Debug.isActive(RemoteDebug::VERBOSE)) debugI("+++++ now millis: %i  +++ Hour Plot updateHour was done with: %d Watt\r\n", now, hour_plot_avg);
		}
	}
//EHorvat new hour plot new end
	...
	if ((WiFi.softAPgetStationNum() > 0) && (WiFi.getMode() == WIFI_AP)) {   	//if any connection is on AP and in AP mode
	...
	if((WiFi.getMode() == WIFI_AP) && (millis() - Last_APClient_millis > 90000)) {    		// EHorvat check if no Client did connect to the AP in last 90 sec
		Last_APClient_millis = millis();
		if(Debug.isActive(RemoteDebug::DEBUG)) debugI("++++-----  No Clients where connected to AP in last 90 Seconds .... do a swapWifiMode now ...");
		swapWifiMode();
	}
...
void handlePriceApi(unsigned long now) {
	...
	if(eapi != NULL && ntpEnabled) {
		start = millis();
//		debugW_P(PSTR("++--// Running handlePriceApi ")); // EHorvat debug
	...
	if(gpioConfig.apPin != 0xFF) {
		if (digitalRead(gpioConfig.apPin) == LOW) {
			...
			if ((now - buttonTimer > longPressTime) && (longPressActive == false)) {
				...
				debugD_P(PSTR("Button was long pressed, so do a Wifi Swap ..."));   //EHorvat ... get more info
				swapWifiMode();
			}
		} else {
...
void swapWifiMode() {
	...
	if (mode != WIFI_AP || !config.hasConfig()) {
		...
		Serial.printf(PSTR("  Wifi AP :             '%s'\r\n"), (String("amsreader-") + String(chipId, HEX)).c_str());	    //EHorvat ... always show this info on serial debug
	...
	while(hanSerial->available() && pos == DATA_PARSE_INCOMPLETE) {
		...
		if(ctx.type > 0 && pos >= 0) {
			if(ctx.type == DATA_TAG_DLMS) {
				debugI_P(PSTR("Received valid DLMS at %d"), pos);  //EHorvat set debug level to Info (was debugD_P)
			} else if(ctx.type == DATA_TAG_DSMR) {
				debugI_P(PSTR("Received valid DSMR at %d"), pos); //EHorvat set debug level to Info (was debugD_P)
			} else {
...
void WiFi_connect() {
	...
	if (WiFi.status() != WL_CONNECTED) {
		...
		if(!config.getWiFiConfig(wifi) || strlen(wifi.ssid) == 0) {
			if (Debug.isActive(RemoteDebug::INFO)) debugI_P(PSTR("no valid Wifi config....swapping to AP now.."));  //EHorvat more info
			swapWifiMode();
			return;
		}
...
void WiFi_post_connect() {
	...
	if(config.getWiFiConfig(wifi)) {
		...
		Serial.printf("Successfully connected to WiFi! \n");				//EHorvat ... always show this info on serial console
		Serial.printf("IP:  %s \n", WiFi.localIP().toString().c_str());	    //EHorvat ... always show this info on serial console
		Serial.printf("AP Pin: %d \n", gpioConfig.apPin);	                //EHorvat ... always show this info on serial console
		if(Debug.isActive(RemoteDebug::INFO)) {
...
void configFileParse() {
	...
	debugI_P(PSTR("Saving configuration now...oida"));
	Serial.flush();
