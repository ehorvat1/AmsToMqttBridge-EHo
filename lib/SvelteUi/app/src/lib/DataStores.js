        ...
        fetchWithTimeout("/data.json")
            ...
            .then((data) => {
                ...
                if(sysinfo.upgrading) {
                    ...
                    getSysinfo();
                    if(minutePlotTimeout) clearTimeout(minutePlotTimeout);  //EHorvat new hour plot
                    minutePlotTimeout = setTimeout(getMinutePlot, 2500);    //EHorvat new hour plot
                    if(hourPlotTimeout) clearTimeout(hourPlotTimeout);  //EHorvat new hour plot
                    hourPlotTimeout = setTimeout(getHourPlot, 3500);    //EHorvat new hour plot
...
let minutePlot = {};
let minutePlotTimeout;
export async function getMinutePlot() {
    if(minutePlotTimeout) {
        clearTimeout(minutePlotTimeout);
        minutePlotTimeout = 0;
    }
    const response = await fetchWithTimeout("/minuteplot.json");
    minutePlot = (await response.json())
    minutePlotStore.set(minutePlot);

    let date = new Date();

minutePlotTimeout = setTimeout(getMinutePlot, 5000)  //Update every 5 sec
}

export const minutePlotStore = writable(minutePlot, (set) => { 
    getMinutePlot();
    return function stop() {}
});
...
export async function getHourPlot() {
    ...
    let date = new Date();
    hourPlotTimeout = setTimeout(getHourPlot, 20030)  //Update every 20 sec
}
