...
// EHorvat
    ...
    let ymax = 0;
    let ymin = 0;    
    let no_of_datapoints = 119;   //Number of points-1 in minute Plot 
    let max_exp = 0;
    let min_exp = 0;
    ...
    $: {
        ...
        for(i = no_of_datapoints; i>-1; i--) {
            ...
            if ((i+1)%6 == 0) {                     //show x ticks not on every data point
                xTicks.push({
                    label: zeropad(-(i+1)/12)      //show "-" Seconds
                });
            } else {
            ...
            points.push({
                label: imp,
                title: 'vor ' + ((i+1)*5) + ' Sek.: ' + imp + ' W',
                value: imp, 
                label2: exp, 
                title2: 'vor ' + ((i+1)*5) + ' Sek.: ' + (- 1 * exp)+ ' W',
                value2: exp,
                color2: '#407038',
                color: '#7c3aed' 
            }); 
...
//     
        ...
        config = {
            title: "Elektrische Leistung letzte 10 Minuten (Watt)",
            height: 226,
            width: 1520,
