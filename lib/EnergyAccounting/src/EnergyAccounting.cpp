...
bool EnergyAccounting::update(AmsData* amsData) {
    ...
    if(kwhe > 0) {
        ...
        produce += kwhe;
        if(incomerefund != ENTSOE_NO_VALUE) { //EHorvat new fixed income price / refund was...
...
//            incomeDay += price * (wh / 1000.0);            // EHorvat Moved to next if getIncomeRefundForHour
    ...
    if(getIncomeRefundForHour(0) != ENTSOE_NO_VALUE) {       //EHorvat new fixed income price / refund
        for(uint8_t i = 0; i < currentHour; i++) {
