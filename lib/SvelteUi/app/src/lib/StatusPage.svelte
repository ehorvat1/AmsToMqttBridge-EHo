...
<div class="grid xl:grid-cols-5 lg:grid-cols-3 md:grid-cols-2">
    ...
    <div class="cnt">
        ...
        <div class="my-2"><!-- EHorvat next line was :   Installed version: {sysinfo.version} -->
            Installed Version: 2.2.21-EHo-en
        </div>        
        <div class="my-2"><!-- EHorvat new info-->
            Date of Firmware: 7. Sept. 2023
        </div>
<!--  EHorvat disabled consent stuff  
        ...
        <div class="my-2 flex">
            <form action="/firmware" enctype="multipart/form-data" method="post" on:submit={() => firmwareUploading=true} autocomplete="off">
                ...
                <button type="submit" class="btn-pri-sm2 float-right">Upload now</button>
                {/if}
            </form>
        </div>
        {/if}
    ...
    <div class="cnt">
        ...
        <form action="/configfile" enctype="multipart/form-data" method="post" on:submit={() => configUploading=true} autocomplete="off">
            ...
            {:else}
            {configFiles[0].name}
            <button type="submit" class="btn-pri-sm2">Upload now</button>
            {/if}
        </form>
    </div>
    {/if}
</div><!-- EHorvat next line was :   Uploading firmware, please wait -->
<Mask active={firmwareUploading} message="Firmware is currently being uploaded, should take app. 30 sec. Do not close browser window."/>
<Mask active={configUploading} message="Currently updating configuration, please wait ..."/>
