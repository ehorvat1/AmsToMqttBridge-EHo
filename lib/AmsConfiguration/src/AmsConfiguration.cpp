...
bool AmsConfiguration::setEntsoeConfig(EntsoeConfig& config) {
	...
	if(getEntsoeConfig(existing)) {
		...
		entsoeChanged |= config.fixedRefund != existing.fixedRefund;                           //EHorvat new fixed income price / refund
		entsoeChanged |= config.last_counter_value != existing.last_counter_value;             //EHorvat new last_counter_value
		entsoeChanged |= strcmp(config.last_counter_info, existing.last_counter_info) != 0;    //EHorvat new last_counter_value
		entsoeChanged |= config.last_expcounter_value != existing.last_expcounter_value;             //EHorvat new last_counter_value
		entsoeChanged |= strcmp(config.last_expcounter_info, existing.last_expcounter_info) != 0;    //EHorvat new last_counter_value
	} else {
		entsoeChanged = true;
	}
...
void AmsConfiguration::clearEntsoe(EntsoeConfig& config) {
	...
	config.fixedPrice = 0;
	config.fixedRefund = 0;                 //EHorvat new fixed income price / refund
	config.last_counter_value = 0;          //EHorvat new last_counter_value
	strcpy(config.last_counter_info, "");   //EHorvat new last_counter_value
	config.last_expcounter_value = 0;          //EHorvat new last_counter_value
	strcpy(config.last_expcounter_info, "");   //EHorvat new last_counter_value
...
void AmsConfiguration::clearUiConfig(UiConfig& config) {
	...
	config.showPeaks = 0;		//EHorvat dont show peaks by default (was 2)
	config.showPricePlot = 0;	//EHorvat dont show prices by default (was 2)
	config.showMinutePlot = 1;	//EHorvat new minute plot
	config.showHourPlot = 1;	//EHorvat new hour plot
	config.showDayPlot = 1;
	config.showMonthPlot = 1;
	config.showTemperaturePlot = 0;//EHorvat dont show temp by default (was 2)
}
...
bool AmsConfiguration::hasConfig() {
	...
	if(configVersion > EEPROM_CHECK_SUM) {
		...
		switch(configVersion) {
			...
			case 103:				//EHorvat for new fixed income price / refund   and last_counter_value
				configVersion = -1; // Prevent loop
...
bool AmsConfiguration::relocateConfig103() {		//EHorvat new fixed income price / refund  and last_counter_value
	EEPROM.begin(EEPROM_SIZE);
	...
	MqttConfig mqtt;
	EEPROM.get(1004, mqtt);

	HomeAssistantConfig haconf;
	EEPROM.get(1680, haconf);
