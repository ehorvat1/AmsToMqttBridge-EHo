...
<div class="container mx-auto m-3">
  ...
  {#if sysinfo.upgrading}<!-- EHorvat next line was :   Device is upgrading, please wait -->
  <Mask active=true message="Firmware wird jetzt ge&auml;ndert, bitte warten ... ca. 45 Sekunden"/>
  {:else if sysinfo.booting}
    {#if sysinfo.trying}<!-- EHorvat next line was :   Device is booting, please wait. Trying to reach it on -->
      <Mask active=true message="Microprozessor wird neu gestartet, bitte warten. Verbinde zu {sysinfo.trying}"/>
    {:else}<!-- EHorvat next line was :   Device is booting, please wait-->
      <Mask active=true message="Microprozessor wird neu gestartet, bitte warten."/>
    {/if}
  {/if}
</div>
