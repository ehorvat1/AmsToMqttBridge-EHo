<script>
    // Generate a plot for 1 hour duration
    ...
    // EHorvat
        ...
        let ymax = 0;
        let ymin = 0;    
        let no_of_datapoints = 119;   //Number of points-1 in Hour Plot (120 points ...2 per minute)
        let max_exp = 0;
        let min_exp = 0;
        ...
        $: {
            ...
            for(i = no_of_datapoints; i>-1; i--) {
                ...
                if ((i+1)%4 == 0) {                     //show x ticks not on every data point
                    xTicks.push({
                        label: zeropad(-1*(i+1)/2)      //show "-" Minutes
                    });
                } else {
                ...
                points.push({
                    label: imp,
                    title: ((i+1)/2).toFixed(1) + ' min. ago: ' + imp + ' W',
                    value: imp, 
                    label2: exp, 
                    title2: ((i+1)/2).toFixed(1) + ' min. ago: ' + (- 1 * exp)+ ' W',
                    value2: exp,
                    color2: '#407038',
                    color: '#7c3aed' 
                }); 
    ...
    //     
            ...
            config = {
                title: "Electric power last hour (Watt)",
                height: 226,
                width: 1520,
