            ...
            <g class='bars'>
                {#each config.points as point, i}
                 ...
                 {#if (!isNaN(xScale(i)) && !isNaN(yScale(point.value))) || !isNaN(xScale(i)) && !isNaN(yScale(point.value2))}    
                        <g>
                        {#if point.value !== undefined}   <!-- EHorvat changed: x="{xScale(i) + 2}"  width="{barWidth - 4}" -->
                            <rect
                                x="{xScale(i) + 1}"
                      ...
                      <!-- EHorvat 1 hour plot end -->     
                        ...
                        <!-- EHorvat changed: x="{xScale(i) + 2}"  width="{barWidth - 4}" -->
                        <!-- EHorvat changed: height="{yScale(config.y.min) - yScale(config.y.min + point.value2)}" -->
                        <!-- EHorvat changed fill="{point.color}" to fill="{point.color2}" -->
                            <rect
                                x="{xScale(i) + 1}"
                                y="{yScale(0)}"
                                width="{barWidth - 2}"
